...
import { materialsPriceList, modifiersPriceList, packingPrice } from 'data/priceListData/index';
...
const getSelectedMaterialPrice = (selectedMaterialType: string | undefined, size: number, amount: number): number => {
  if (!selectedMaterialType) return 0;
  const comparisonOfMaterialType = [...materialsPriceList].filter((item: any) => item.type === selectedMaterialType);
  const materialType = comparisonOfMaterialType.filter((item) => item.size === size && item.amount >= amount)[0];
...
const calculateDefaultPrice = (amount: number, material: string, width: number, height: number): number => {
  const selectedWidth = Number(width);
  const selectedHeight = Number(height);
  const selectedAmount = Number(amount);
  const selectedMaterial = materials.filter((item) => item.value === material);

  if (!selectedMaterial[0] || amount === 0 || selectedWidth * selectedHeight === 0) return 0;

  const materialModifier = getMaterialModifier(selectedMaterial[0].priceModifier);
...
const defaultPrices = (data: CardDescription[], onlyForOnePiece: boolean): number[] => {
  let prices: number[] = [];
  data.map((item, index) => {
    const material = (item.materials.length ? item.materials[0].field : '').toString();
    const price = calculateDefaultPrice(Number(item.amount), material, item.width, item.height);
...
export const isMoreThanMaximumSize = (data: CardDescription[], index: number): boolean => {
  const limits = sizes.map((item) => item.size);
...
export const getPriceForOnePieceOfSection = (data: CardDescription[], index: number): number => {
  const sectionForms = [...data];
...
export const getPriceForSection = (data: CardDescription[], index: number): number => {
  const sectionForms = [...data];
  const calculatorPrice = defaultPrices(sectionForms, false)[index];
  const customPrice =
    sectionForms[index]?.priceForOnePiece * sectionForms[index]?.amount + calculatePackingPrice(sectionForms[index]);

  const sum = isMoreThanMaximumSize(sectionForms, index) ? customPrice : calculatorPrice;
...
export const getSelectedSizeName = (data: CardDescription[], index: number): string => {
  ...
  const selectedSize = sizesOptions.find(
    (option) => option.size === getSizeModifier(sectionForms.width, sectionForms.height)
  );
...
export const isDisplayFabric = (data: CardDescription): boolean => {
  const selectMaterial = data?.materials.map((material) => (data?.materials.length ? material.field : ''))[0];
...
export const getTotalPrice = (data: CardDescription[]): number => {
  ...
  const totalDefaultPrice = Number(
    defaultPrices(sectionForms, false).reduce((accumulator, currentValue) => accumulator + currentValue, 0)
  );
  const totalCustomPrice = Number(
    customPrices(sectionForms).reduce((accumulator, currentValue) => accumulator + currentValue, 0)
  );
