...
const NestedMaterialsForm: React.FC<NestedMaterialsFormProps> = ({
  ...
  const selectedMaterialsArray = [...materials]
    .filter((material) => [...checkedItems].includes(material.value))
    .sort((a, b) => {
  ...
  return (
    <div className={styles.materialsList}>
      ...
      <Popup title={'Dodaj materiał'} trigger={popupTrigger} closePopup={() => setPopupTrigger(false)}>
        <div>
          <div className={styles.selectedMaterials}>
            <p>Wybrane materiały:</p>
            {selectedMaterialsArray.map((item: any, index: number) => (
              <span key={index} style={{ border: `${index === 0 && '2px solid green'}` }}>
                {' '}
                {item.value}
                <button style={{ marginLeft: 4 }} type='button' onClick={() => handleRemoveMaterial(item.value)}>
                  X
                </button>
              </span>
            ))}
          </div>
          <MaterialsList options={materials} checkedItems={checkedItems} handleChange={handleCheckedMaterial} />
        </div>
